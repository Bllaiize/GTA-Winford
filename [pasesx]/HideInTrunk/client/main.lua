...
Citizen.CreateThread(function()
    ...
    while true do
		...
		--Lockstatus End
        if DoesEntityExist(vehicle)  -- remplacer la condition getpedsinvehicleseat par IsVehicleSeatFree(vehicle,-1) pour eviter de se cacher quand quelqu'un est au volant 
		then
                            ...
                            DrawText3D(coords, '[E] Se cacher \n[H] Ouvrir')
								if IsControlJustReleased(0, 74)then
									...
									elseif lockStatus == 2 then -- locked
										ESX.ShowNotification('Voiture fermé')
									end
								end
                        else
                            DrawText3D(coords, '[E] Se cacher \n[H] Ouvrir')
                    ...
                    if IsControlJustReleased(0, 38) and not inTrunk then
						...
						if lockStatus == 1 then --unlocked
							if DoesEntityExist(playerPed) then
								if not IsEntityAttached(playerPed) or GetDistanceBetweenCoords(GetEntityCoords(playerPed), GetEntityCoords(PlayerPedId()), false) >= 2.0 then
                                    print("1")
                                    print(IsSeatWarpOnly(vehicle,-1))
									SetCarBootOpen(vehicle)
									Wait(350)
